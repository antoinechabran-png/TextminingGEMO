...
with st.sidebar:
    ...
    if enable_crush:
        ...
        if crush_dict_file:
            crush_sheet = st.selectbox("Select Crush Language Sheet", 
                                     ["English", "French", "English For Translation"])
    st.divider()
    ...
    if 'processed_emo' in st.session_state:
        ...
        with tab2:
            ...
            if all_matches:
                ...
                with c1:
                    ...
                    ax_cat.bar(cat_counts.index, cat_counts.values, color=['#FFADAD', '#A0C4FF', '#CAFFBF'])
                    st.pyplot(fig_cat)
                with c2:
                    sub_counts = m_df.groupby(['cat', 'sub']).size().reset_index(name='count').sort_values('count')
                    fig2, ax2 = plt.subplots()
                    ax2.barh(sub_counts['sub'], sub_counts['count'], color='#A0C4FF')
                    st.pyplot(fig2)
        ...
        with tab4:
            ...
            if enable_crush and 'is_crush' in df.columns:
                ...
                bars_crush = ax_crush.barh(crush_data.index, crush_data.values, color='#FFADAD')
